...
alphabet = ["a", "b", "c", "d", "e", "f", "g", "h", "i"] #Sert pour les noms des colonnes
...
def creation_liste_clauses(grille_initiale: List[str], liste_position: List[str], dict_cases: dict):
    ...
    liste_clauses_final += [cl for cl in liste_clauses_ligne if cl not in liste_clauses_final]
    liste_clauses_final += [cl for cl in liste_clauses_colonne if cl not in liste_clauses_final]
    liste_clauses_final += [cl for cl in liste_clauses_carre if cl not in liste_clauses_final]
...
def afficher_grille(grille : List[List[int]], init : bool = False, liste_position : List[str] = None):
    ...
    if init:
        ...
        grille_affichage_trieer = [grille_affichage_trieer[i:i + 9] for i in range(0, len(grille_affichage_trieer), 9)]
        for i in range(len(grille_affichage_trieer)):
            grille_affichage_trieer[i] = sorted(grille_affichage_trieer[i], key=lambda x: x[1])
...
def solver():
    ...
    liste_clauses = creation_liste_clauses(grille_initiale, liste_position, dictionnaire_cases)
